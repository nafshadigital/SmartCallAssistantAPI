...
/*
	$query = "select name,concat(country_code,mobile) as mobile from tbl_users where id=$senderid";
	$sender = $m -> executeQuery($query,'select');

	$sender_name	= $sender[0][0];
	$sender_phone	= $sender[0][1];	
*/
	...
	{
		$query = "select mobile from tbl_users where id='$senderid'";
		$sender_name = $m -> executeScalar($query);
		if(strlen($sender_name) < 2)
		{
		$resultObj -> status   = 0;
	...
	if ($err) 
	{
		...
		$resultObj -> message  = "Heart Sent Successfully !".$sender_name;
